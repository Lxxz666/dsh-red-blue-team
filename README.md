...
> 基于 [dsh-python](https://github.com/Lxxz666/dsh-python)（DeepSeek Harness Python 实现）二次开发的**红队/蓝队智能安全检测系统**：
> **主 Agent 派发子 Agent 并行攻击你的业务系统 → 确定性判定漏洞 → 出攻击报告 → 自动修复 → 回归验证清零 → 用 wanter 势能地形学会"最有效的攻击"。**
>
> 一个命令跑完整闭环，一个文件夹或一个网址即可开测，开箱即用、零 API Key 依赖。
...
| 上线前的焦虑 | 本项目的答案 |
|:--|:--|
| "我的 AI 客服会被提示注入打穿吗？" | 37 类攻击向量自动打一遍，注入/提权/投毒**当场演示** |
...
| "发现漏洞后怎么修？修没修好？" | 65 类修复模板（问题说明+代码级 before/after）→ 沙箱修复 → **回归清零** |
...
| 测试规模 | **314 passed**（框架 224 + 红蓝队 90） | 判定/靶场/场景/静态/多Agent/回归全链路 |
| 扫描确定性 | 两次扫描命中集合**完全一致** | 状态型样本串行通道+重置隔离，报告可复现审计 |
| 自适应收益（wanter 地形） | 25% 预算下命中率 **+10.5%**，覆盖效率提速 **+9.0%** | bench 命令：随机基线 vs 地形序二次扫描对比 |
| 检测面 | **68 条基础样本 × 51 个攻击类别** | D1 Web / D2 API / D3 LLM / D7 配置 + D19 业务场景 |
...
AttackOrchestrator（主 Agent）
  ...
  ▼
BlueEngine（蓝队）：65 类修复模板 → 沙箱应用 → 回归清零 → 完整修复报告
```
...
```
dsh-red-blue-team/
├── dsh/                 # dsh-python 框架完整副本（插件内核/wanter/llm/session…）
├── redteam/             # 红蓝队层：agents(主/子Agent) scenarios(12场景) static(代码审计)
│                        #   vectors(样本库) detector(判定) blueteam(修复) reporter(报告)
│                        #   adaptive(wanter地形) engine storage audit runtime config models
├── target_lab/          # 内置靶场（37 埋入漏洞 + 业务场景 API + 可修复 guards）
├── sample_bank/         # 68 条攻击样本（YAML，四大检测面 + 12 业务场景）
├── tests/ + tests_redteam/  # 314 项测试（含发现率≥80%、零误报、回归清零验收）
├── examples/  docs/     # 示例配置 / 架构·用户·攻击目录·场景·合规文档
└── README.md
```
...
- [ ] V5 MCP 目标适配（dsh.mcp 直连 agent 工具面）
- [ ] V6 LLM 驱动的攻击链编排（子Agent 升级为完整 agent loop）
- [ ] V7 Web 面板（复用 dsh FastAPI）
