...
| "agent 反复失败空转怎么办？" | "重跑" | 🎯 **Reflexion 失败反思**：turn 失败→LLM 生成第一人称教训→写长时记忆→下次同类任务直接引用避免重蹈（论文2303.11366，`REFLECTION_ENABLED=1`） |
...
- **🔒 合规红线**：CLI 扫描非本地目标需 `authorization` 声明；Web 面板任意网址直接输入（本地测试便利）；蓝队只改沙箱，绝不直改生产
...
自行承担法律责任；CLI 扫描保留授权闸门（非本地目标需 `authorization` 声明），
Web 面板为本地测试便利直接输入网址即可。
