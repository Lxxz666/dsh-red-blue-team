...
│         target_lab 内置靶场（HTTP + SDK 双接入，37 埋入漏洞） │
└─────────────────────────────────────────────────────────────┘
```
...
| `dsh.llm.LlmRuntime/MockAdapter/DeepSeekAdapter` | `runtime.py` | LLM 接缝：mock 默认离线确定性；有密钥自动挂 DeepSeek（弱裁判/态势综述） |
...
- `test_scan_e2e`：靶场发现率 **≥80%**（实测 37/37）；全加固靶场 **0 命中**；两次扫描一致；
...
- MCP 目标适配（`dsh.mcp` 客户端直连 agent 系统工具面）；
