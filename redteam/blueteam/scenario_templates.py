...
# ---- 其余 8 大场景（靶场可自动修复：guard 键 + 人工实施指引） ----
# (类别, 标题, 问题与修复说明, guard 键)
_GENERIC_SCENARIOS = [
...
for _category, _title, _how, _guard_key in _GENERIC_SCENARIOS:
    register_template(FixTemplate(
        ...
        auto_fixable=True, guard_key=_guard_key, guard_value=True,
        verify_steps=[_how.split("修复：")[-1], "同一攻击复测被拒绝"]))
